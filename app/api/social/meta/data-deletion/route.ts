  ...
  for (const account of matched) {
    await prisma.socialAccount.update({
      ...
      data: {
        ...
        providerMeta: { facebookUserId: parsed.user_id, deletionCode: confirmationCode },
      },
    });
  }
  logger.info({ facebookUserId: parsed.user_id, confirmationCode }, "meta data deletion requested");
  const url = new URL(request.url);
  return NextResponse.json({
    url: `${url.origin}/studio/accounts?deletion=${encodeURIComponent(confirmationCode)}`,
