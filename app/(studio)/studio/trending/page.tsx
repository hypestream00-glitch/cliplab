    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2 text-[13px]">
        {["ALL", ...TRENDING_PLATFORMS].map((item) => (
          <Link key={item} href={href({ platform: item })} className={`rounded-lg border px-2 py-1 ${platform === item ? "border-primary/60 bg-primary/15" : ""}`}>
            {item === "ALL" ? "Todos" : item}
          </Link>
        ))}
      </div>
      <div className="mb-4 flex flex-wrap gap-2 text-[13px]">
        {["ALL", ...TRENDING_CATEGORIES].map((item) => (
          <Link key={item} href={href({ category: item })} className={`rounded-lg border px-2 py-1 ${category === item ? "border-primary/60 bg-primary/15" : ""}`}>
      ...
      <div className="mb-4 flex flex-wrap gap-2 text-[13px]">
        {[
          ...
          <Link key={id} href={href({ sort: id })} className={`rounded-lg border px-2 py-1 ${sort === id ? "border-primary/60 bg-primary/15" : ""}`}>
            {label}
          </Link>
        ))}
      </div>
