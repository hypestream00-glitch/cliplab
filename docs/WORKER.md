...
- Logs de prontidão (sem secrets): `WORKER STARTED`, `DATABASE: OK`, `REDIS: OK`, `BULLMQ: READY`, `FFMPEG: OK`, `WORKER READY`.
- Diagnóstico de fila: job `cliplab-healthcheck` na queue `healthcheck` (SELECT 1 + Redis, sem créditos, OpenAI, social, email ou projetos).
